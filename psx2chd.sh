...
function compressRoms() {
    ...
    do
	 ...
	 echo "50" 2>&1 |dialog --gauge "Compressing "$box"" 10 70
	 sh $CHD_SCRIPT 2>&1| sed 's/Compressing, //' | sed 's/.'[0-9]'% complete... (ratio='[0-9][0-9]'.'[0-9]'%)//'| dialog --gauge "Compressing "$box"" 10 70
